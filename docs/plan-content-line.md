...
> 상태: **승인 완료 · 구현 완료** (2026-08-06). 결정 ②는 **A안**(280을 하드 캡으로 두지 않음),
> ③ 카테고리 배분 승인, ④ `category_tag`는 현행처럼 **짧은 태그** 사용으로 확정.
...
- [x] `data/phrases.json` 전 항목이 `Phrase` 스키마와 일치(누락 키 0). 스키마 검증 **PASS** — 48건
- [x] 48건, `id` 1~48 연속·중복 없음, 전부 `is_published:false` / `published_at:null`
- [x] 렌더 결과에 💡 / 🇰🇷→🇯🇵 / 🔗 홈페이지 / 📱 LINE **4요소 모두** 출력 — 48건 전수 누락 0
- [x] 전 표현에서 필수 정보·2개 URL·해시태그 보존 — 손실 0건.
      `len()` 기준 216~253으로 `MAX_LENGTH=280` 미만이라 축약 로직은 발동하지 않음
- [x] `LINE_ADD_URL` 미설정 환경에서 기본값(`https://lin.ee/8Rz047O`)으로 정상 렌더
- [x] README §6 스키마 예시에 `nuance` 반영
- [x] `python run_once.py`가 예외 없이 완주 (인증 없어 게시는 실패, 종료코드 1 — 정상)

### ⚠️ 구현 후 실측 — 게시 전 확인이 필요한 사항

새 데이터 48건의 **실제 weighted length는 325 ~ 401**(중앙값 353)이다.

| 근거 | weighted |
|---|---|
| 실제 게시에 성공한 유일한 실적 (id=40, 2026-08-06) | **298** |
| 새 템플릿 48건 최소값 | **325** |
| 새 템플릿 48건 최대값 | **401** |

즉 **새 게시물은 전부 실증된 상한(298)을 넘는다.** A안은 "280이 이 계정의 상한이 아니다"라는
실측에 근거하지만, 그 실측이 보증하는 범위는 298까지다. 401까지 통과한다는 증거는 아직 없다.

게시가 거부되더라도 `is_published`가 갱신되지 않아 **데이터 손상은 없고** 로그에 남으므로,
첫 예약 실행 결과로 판별된다. 다만 상한을 넘겼다면 그때부터 매시간 실패하므로,
**첫 실행 로그를 반드시 확인**할 것. 실패 시 대응은 계획서 §2-4의 B안(템플릿 축소)이다.
